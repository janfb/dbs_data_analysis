...
# load data 
data = ut.load_data_analysis('psd_maxamp_theta_beta_linear.p')
...
std = np.std(np.vstack((data['psd_theta'], data['psd_beta'])), axis=0)
plt.fill_between(frequs[mask], joined_psd_mean[mask] + std[mask], joined_psd_mean[mask] - std[mask], alpha=.2,
                 color='C3', label='+-std')
...
plt.savefig(os.path.join(SAVE_PATH_FIGURES, 'figure_1A_linearized.pdf'))
plt.show()
...
plt.ylabel('count')
plt.legend()
plt.savefig(os.path.join(SAVE_PATH_FIGURES, 'figure_1B_linearized.pdf'))
plt.show()
